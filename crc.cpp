...
unsigned long ComputeCRC32Part(unsigned char *buf,int len,unsigned long  crc)
{
   for(int i=0;i<len;i++)
   {
		int index=(crc^(*buf++))&0xff;
		crc=((crc>>8)^CRC32Table[index]);
   }
	return crc;
}

unsigned long ComputeCRC32(unsigned char *buf,int len)
{
	return ~ComputeCRC32Part(buf,len,0xffffffff);
}
