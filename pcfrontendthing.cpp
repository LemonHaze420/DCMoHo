...
// $Log$
// Revision 1.1  2000-08-05 10:33:23+01  jjs
// First PC milestone.
//
...
void	CFrontEndThing::SetAnimMode(EAnimMode am)
{
	...
	if (mMesh.mMesh)
	{
		mFrameInc = mMesh.mMesh->GetFrameIncrement(mAnimMode, &mAnimIndex);
	}
}
...
//******************************************************************************************
void	CFrontEndThing::Render()
{
	FVector f;
	FMatrix fm;
	GVector g = GetRenderPos();
	GVectorToFVector(&g,&f);
	GMatrix gm = GetRenderOrientation(0);
	GMatrixToFMatrix(&gm, &fm);

	// Correct aspect ratio here?
	
	mMesh.RenderFEMesh(f,fm,this);
}
