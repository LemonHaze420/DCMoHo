...
//******************************************************************************************
void	CMeshAccumulatorDC::AddMesh(CMesh *m)
{
...
//******************************************************************************************
SINT	CMeshAccumulatorDC::GetMeshIndex(CMesh *m)
{
	if (!m)
		return -1;
...
//******************************************************************************************
void	CMeshAccumulatorDC::AddVertex(FVector &v)
{
	if (mMAMesh.mNumVertices < MADC_MAX_MA_VERTICES)
	{
		SINT	c0;
...
//******************************************************************************************
void	CMeshAccumulatorDC::AddNormal(FVector &v)
{
	if (mMAMesh.mNumNormals < MADC_MAX_MA_NORMALS)
	{
		SINT	c0;
...
//******************************************************************************************
SINT	CMeshAccumulatorDC::GetVertex(FVector &v)
{
	SINT	c0;
...
//******************************************************************************************
SINT	CMeshAccumulatorDC::GetNormal(FVector &v)
{
	SINT	c0;
...
void	fix_max_uvs_DC(float &uv0, float &uv1, float &uv2)
{
...
//******************************************************************************************
void	CMeshAccumulatorDC::CompileAndWriteMeshes(CTextureAccumulatorDC &ta, CMEMBUFFER &mem)
{
	...
	for (c0 = 0; c0 < mNumMeshes; c0 ++)
	{
		...
		for (c2 = 0; c2 < m->mNumParts; c2 ++)
		{
			...
			{
				SMADCGTriangle		*gt  = &mMAMesh.mGTriangles[c1];
				...
				{
					SMADCTTriangle		*tt  = &mMAMesh.mTTriangles[sk][c1];

					// get texture reference from texture accumulator
					STADCTexture	*tex = ta.GetTextureRef(TADCTT_MESH, mt->mA->mTexture[sk]);

					if (tex)
					{
						// first. generic texture information
						tt->mTPageNo = tex->mPage;
						...
						fix_max_uvs_DC(u0, u1, u2);
						fix_max_uvs_DC(v0, v1 ,v2);

						// push the uvs a bit to prevent seepage
						float	texx = tex->mX + 0.002f;
						float	texy = tex->mY + 0.002f;
						float	texw = tex->mW - 0.004f;
						float	texh = tex->mH - 0.004f;

						tt->mU0 = texx + (texw * u0);
						tt->mV0 = texy + (texh * (1.f - v0));
						tt->mU1 = texx + (texw * u1);
						tt->mV1 = texy + (texh * (1.f - v1));
						tt->mU2 = texx + (texw * u2);
						tt->mV2 = texy + (texh * (1.f - v2));
			...
			for (c3 = 0; c3 < mp->mNumAFrames; c3 ++)
			{
				for (c1 = 0; c1 < mp->mNumPVertices; c1 ++)
				{
					...
					tempmem.Write(&mp->mPVertices[c3][c1], 12);
				}
			}
			...
			{
				// convert to dest type
				tempmem.Write(&mMAMesh.mNormals[c1].mNormal, 12);
			}
			...
			{
				tempmem.Write(&mMAMesh.mGTriangles[c1], sizeof(SMADCGTriangle));
			}
			...
			for (sk = 0; sk < m->mNumSkins; sk ++)
			{
				for (c1 = 0; c1 < mMAMesh.mNumTriangles; c1 ++)
				{
					tempmem.Write(&mMAMesh.mTTriangles[sk][c1], sizeof(SMADCTTriangle));
				}
			}
			...
			tempmem.Write(&mp->mOffsetPos, 12);
			tempmem.Write(&mp->mOrientation, sizeof(FMatrix));
		...
		for (c1 = 0; c1 < m->mNumEmitters; c1++)
		{
			...
			tempmem.Write(&m->mEmitters[c1].mPos, 12);
