...
typedef struct {
#ifdef JCLDCNOPSX		// This doesn't seem to be used for anything...
...
//sprintf(temp,"File opened %s ",fname);
//mw_pr(temp);
...
/*
	...
	if(!CdSearchFile(&OpenHandle,fname2))
		return	-1;                     
*/
	sprintf(temp, "\\MOHODC\\%s", fname);
	OpenHandle.size=sDCOpenFile(temp);
...
//gdFsRead(OpenHandle.gdfs);
	...
	if(StartedReading==0)
	{
	   result=sDCLoadPartFile(buffer,len);
	//	result=CdReadFile(fname2,buffer,(len+2047)&~2047);
		StartedReading=1;
	}
	else 
	{
	result=sDCLoadPartFile(buffer,(len+2047)&~2047);
	}
...
SINT	DCLoadFile(char *fname, ULONG *buffer)
{
	char fname2[256];
	RedirectCDAccess();
