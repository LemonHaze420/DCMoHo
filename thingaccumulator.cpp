...
//******************************************************************************************
void	CThingAccumulator::AddThing(SINT oid)
{
	if (oid < 0) 
	{
		ASSERT(0);
...
//******************************************************************************************
void	CThingAccumulator::CompileAndWrite(CTextureAccumulator &ta, CMeshAccumulator &ma, CMEMBUFFER &mem)
{
		...
		else
		{
			if (rt)
			{
				...
				while (p)
				{
					...
					p->FillOutPSXRenderInfo(rd.mRTInfos[rd.mNumRenderThings], ta, ma, pcrd->mInit[rd.mNumRenderThings]);
